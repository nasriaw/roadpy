...
# c = fmax              
# d = R design (m)                      
# e = waktu tempuh melintas lengkung peralihan, bisa diambil = 3 "
# Dd = 
# ed = 
# f = delta_i, tingkat perubahan kelandaian melintang: 0.035 (Vr<80) atau 0,025 (Vr>80), ref: SE hal 100
# g = Angle PI, derajat
...
def alignmentHorizontal():
    ...
    data2=data1.assign(PI_angle=df1) #data 2 gabungan data1, using assign() method to add a new column
    data3=(data2.iloc[0:3,4]) #sudut_PI, baris 0-
    ...
    df=df.join(data_geometric, lsuffix="left") #digabung data input geometrik dan hasil perhitungan jarak dan sudut
    print()
    print('4. Calculation Results as Geometric Planning Input Data:')
    print(df)
    df.to_csv("design_input_data.csv") # data disimpan untuk perhitungan bagian geometrik

    #3.4. CALCULATING GEOMETRIC ELEMENTS 
    output=[]
    for i in range(len(df)):
         Fmax=hitungFmax(df.iloc[i,1])                                # () kec. rencana
         Rmin=hitungRmin(df.iloc[i,1], df.iloc[i,2], df.iloc[i,3])    # ( , , ) = (Kec.rencana, e_max, f_max )
         Dmax=hitungDmax(df.iloc[i,1], df.iloc[i,2], df.iloc[i,3])    # ( , , ) = (Kec.rencana, e_max, f_max )
         Dd=hitungDd(df.iloc[i,4])                                    # () = (R_rencana)
         e_hitung=e_Hitung_def(df.iloc[i,2], Dd, Dmax)                    # ( , , ) =( e_max, , )
    ...
    img1 = mpimg.imread('Figure Detail of Alignment Horizontal.png') #Figure Alignment Horizontal.png, figure_element_alignment_horizontal.png
